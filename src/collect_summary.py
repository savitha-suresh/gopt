...
for i in range(0, 10):
    ...
    if os.path.isfile(cur_exp_dir + '/result.csv'):
        try:
            ...
            with open(cur_exp_dir + '/result.csv') as f:
                reader = csv.DictReader(f)
                headers = reader.fieldnames
            #print("ress - {}".format(cur_res))
...
header = ",".join(headers)
np.savetxt(args.exp_dir + '/result_summary.csv', [result_mean, result_std], delimiter=',', header=header)
